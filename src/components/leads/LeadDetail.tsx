...
import { ArrowLeft, Check, Clock, DollarSign, Mail, MessageCircle, Pencil, Phone, RotateCcw, Trash2, X, BellRing } from "lucide-react";
...
export function LeadDetail({
  ...
  // ---- Shared blocks, rendered by both the desktop side-drawer and the
  // mobile full-screen view below, so nothing drifts between the two. ----

  const scoreOverrideControls = editingScore ? (
  ...
  const followUpBanner = isActiveLead(lead) && status && (
    <div
  ...
  const editFormBlock = editData && (
    <div className="space-y-3 mb-5 p-4" style={CARD}>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <FieldLabel>Phone</FieldLabel>
          <input value={editData.phone} onChange={setF("phone")} className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle} />
        </div>
        <div>
          <FieldLabel>Email</FieldLabel>
          <input value={editData.email} onChange={setF("email")} className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle} />
        </div>
      </div>
      <div>
        <FieldLabel>Lead type (bin)</FieldLabel>
        <select value={editData.source} onChange={setF("source")} className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle}>
          {SOURCES.map((s) => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <select value={editData.timeline} onChange={setF("timeline")} className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle}>
        ...
        <div>
          ...
          <select value={editData.hasAgent} onChange={setF("hasAgent")} className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle}>
            {AGENT_OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
      ...
      <div>
        ...
        <input type="number" min="0" value={editData.dealValue} onChange={setF("dealValue")} placeholder="Only once under contract" className="w-full px-3 py-2.5 text-sm outline-none" style={inputStyle} />
      </div>
      <div>
        <FieldLabel>Notes</FieldLabel>
        <textarea value={editData.notes} onChange={setF("notes")} rows={2} className="w-full px-3 py-2.5 text-sm outline-none resize-none" style={inputStyle} />
      </div>
      <div className="flex gap-2 pt-1">
        <PrimaryButton onClick={saveEdit} className="flex-1 py-2.5">
          Save changes
        </PrimaryButton>
        <button onClick={() => setEditing(false)} className="press px-4 py-2.5 text-sm font-medium" style={{ color: COLORS.inkSoft }}>
          Cancel
        </button>
      </div>
    </div>
  ...
  const interactionLogBlock = (
    <div className="mb-6">
      ...
      {templates.length > 0 && (
        <select value={templateId} onChange={(e) => applyTemplate(e.target.value)} className="w-full mb-2 px-3 py-2.5 text-xs outline-none" style={inputStyle}>
      ...
      <div className="flex gap-2">
        ...
        />
        <PrimaryButton onClick={logInteraction} className="px-4 py-2.5">
          Log
        </PrimaryButton>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div
        className="hidden sm:flex fixed inset-0 z-30 justify-end anim-fadein"
        style={{ background: "#0A0A08AA", backdropFilter: "blur(3px)" }}
        onClick={onClose}
      >
      ...
      <div className="sm:hidden fixed inset-0 z-50 flex flex-col anim-fadein" style={{ background: COLORS.bg }}>
        <div
          className="flex items-center justify-between px-2 flex-shrink-0"
          style={{ paddingTop: "max(14px, env(safe-area-inset-top))", paddingBottom: 10, borderBottom: `1px solid ${COLORS.border}` }}
        >
          <button onClick={onClose} aria-label="Back" className="press w-11 h-11 rounded-full flex items-center justify-center" style={{ color: COLORS.ink }}>
            <ArrowLeft size={21} />
          </button>
          {!editing && (
            <button onClick={startEdit} className="press text-sm font-semibold px-3" style={{ color: COLORS.accentBright }}>
              Edit
            </button>
          )}
        </div>

        <div className="flex-1 overflow-y-auto px-5 pt-2" style={{ paddingBottom: "max(24px, env(safe-area-inset-bottom))" }}>
          {editing && editData ? (
            editFormBlock
          ) : (
            <>
              <div className="flex flex-col items-center text-center pt-3 pb-6">
                ...
                <h2 className="mt-3.5 text-2xl" style={{ fontFamily: "'Fraunces', serif", color: COLORS.ink }}>
                  {lead.name}
                </h2>
                <p className="text-xs mt-1 uppercase tracking-wide" style={{ color: COLORS.inkSoft }}>
                  {lead.source} · {new Date(lead.created_at).toLocaleDateString()}
                </p>
                <div className="mt-3 flex flex-col items-center gap-1.5">
                  <Pill color={bucketColor(bucket)}>{bucket} lead</Pill>
                  {lead.manual_score != null && (
                    <span className="text-xs" style={{ color: COLORS.inkSoft }}>
                      overridden (auto: {lead.auto_score})
                    </span>
              ...
              {(lead.phone || lead.email) && (
                <div className="grid gap-2.5 mb-6" style={{ gridTemplateColumns: `repeat(${(lead.phone ? 2 : 0) + (lead.email ? 1 : 0)}, 1fr)` }}>
                  {lead.phone && (
                    <a href={`tel:${lead.phone}`} className="press flex flex-col items-center gap-2 py-4" style={{ ...CARD_SM, minHeight: 72 }}>
                      <Phone size={20} style={{ color: COLORS.accentBright }} />
                  ...
                  {lead.phone && (
                    <a href={`sms:${lead.phone}`} className="press flex flex-col items-center gap-2 py-4" style={{ ...CARD_SM, minHeight: 72 }}>
                      <MessageCircle size={20} style={{ color: COLORS.accentBright }} />
                      <span className="text-xs font-semibold" style={{ color: COLORS.ink }}>
                        Text
                      </span>
                    </a>
                  ...
                  {lead.email && (
                    <a href={`mailto:${lead.email}`} className="press flex flex-col items-center gap-2 py-4" style={{ ...CARD_SM, minHeight: 72 }}>
                      <Mail size={20} style={{ color: COLORS.accentBright }} />
                      <span className="text-xs font-semibold" style={{ color: COLORS.ink }}>
                        Email
                      </span>
                    </a>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {STAGES.map((s) => {
                    ...
                    return (
                      <button
                        key={s}
                        onClick={() => onUpdate({ stage: s }, { silent: true })}
                        ...
                        style={{
                          ...
                          color: active ? "#FBF3EF" : COLORS.inkSoft,
                        }}
                      >
                        {s}
                      </button>
              ...
              {lead.deal_value != null && (
                <div className="flex items-center gap-2 mb-5 text-sm" style={{ color: COLORS.ink }}>
                  <DollarSign size={14} style={{ color: COLORS.inkSoft }} /> ${Number(lead.deal_value).toLocaleString()} gross commission
                </div>
              )}
